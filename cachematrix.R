## This R program source code is for Coursera R programming course programming 
## assignment #2 solution submission. Matrix inversion is usually a costly
## computation and there may be some benefit to caching the inverse of a matrix
## rather than compute it repeatedly The below two functions are written that
...
## value of the mean in the cache via the setinvrs function. This function also
## checks whether the matrix provided is invertible or singular. If singular
## then throws warning message. This is an additional functinality.
...
cacheSolve <- function(x, ...) {
        ...
        if(det(data)==0) {  ## checks if the matrix is invertible or singular
                ## if determinant is 0 then singular
                message("Error!! Sigular matrix provided...")
        }
        else {
                iv <- solve(data, ...)
                x$setinvrs(iv)
